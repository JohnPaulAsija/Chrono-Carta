...
- **`getCuratorClient(session)` replaced with `getServerSupabase()`** at the start of Phase 5. The session-arg factory had zero callers — integration tests inlined the equivalent logic in `setup.ts`, no production code consumed it yet — and the canonical Supabase Next.js SSR pattern reads the session from cookies via `@supabase/ssr`'s `createServerClient`. Replacing with the canonical pattern was a net subtraction of code and brought the codebase in line with the framework's conventions. Architecture §Application Architecture, §Project Structure, §RLS, and CLAUDE.md's two-client section were rewritten in the same commit. The route-group split remains the security boundary; the function-signature guardrail is gone, but it never had a real callsite to enforce.
