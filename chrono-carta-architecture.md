...
- **Map rendering:** `@vnedyalk0v/react19-simple-maps` for GeoJSON polygon rendering with D3 geo projections — a maintained fork of `react-simple-maps` with React 19 peer deps, bundled TypeScript types, and ESM/CJS/UMD builds compatible with Next 16 / Turbopack. Always pass `geojson` as an in-memory object loaded server-side, never as a URL string, to avoid client-side async-fetch hydration mismatches in `Geographies`.
- **Two rendering layers over a tinted container.** SVG paint order is document order, so each layer covers the one before:
  1. The map container `<div>` has `backgroundColor: "#a8d5e2"` — a flat ocean fill covering the entire rectangle of the map area. Visible wherever neither land nor a political polygon sits on top, including the corners of the SVG viewport that fall outside the equal-earth projection's curved bounds. (An earlier design used a `<Sphere>` element drawing the curved globe outline; flattening to a tinted container eliminated the void shown when users panned past the globe's edge, and made the visible "world" a clean rectangle.)
  2. `<Geographies geography={LAND_FEATURES} data-layer="basemap">` — a static, modern-coastline dissolved land multipolygon from `world-atlas` (Natural Earth 1:50m, public domain). Fill `#e8e0c8`, stroke `#a89f80`, `pointerEvents: "none"` so it never participates in highlighting. Identical for every map and every year — hardcoded inside `MapViewer`.
  3. `<Geographies geography={geojson} data-layer="political">` — the per-request Cliopatria polygons. Hover-and-tooltip interactive.
- **Optional fourth layer: modern country borders.** A "Modern borders" checkbox above the map toggles a `<Geographies geography={COUNTRY_BORDERS} data-layer="modern-borders">` block on top of the political polygons, rendering modern country boundaries from `world-atlas`'s `countries-50m.json`. Lines only — `fill="none"`, thin gray stroke at `strokeOpacity` 0.9, `vectorEffect="non-scaling-stroke"`, and `pointerEvents: "none"` so it remains non-interactive. The layer is a pedagogical orientation aid: students can see how a historical entity overlaps modern countries. Default off; toggle state is local to `MapPanel`. In gameplay (when the play route is built) the toggle is treated the same way — ChronoCarta is an ed-tech tool with no leaderboards, scores-for-prizes, or other stakes, so seeing modern borders is *not* a cheat, it's learning support. The toggle should reset to off at the start of every gameplay round; the planned mechanism is `key={roundId}` on the map container, which remounts and re-initializes `useState(false)` so students attempt each map without the crutch first.
- **Why a separate basemap.** Cliopatria contains only political-entity geometry, not coastlines. Without layer 2, land outside any political entity is indistinguishable from ocean, and players have no geographic anchor. Modern coastlines are used for every era; the historical anachronism (Doggerland, the Aral Sea, etc.) is invisible to players for the vast majority of the game's year range.
